---
name: humanizer
description: Removes signs of AI-generated writing from text. Use when editing or reviewing prose, humanizing articles, blog posts, marketing copy, or drafts that sound AI-generated. Detects and fixes inflated symbolism, promotional language, superficial -ing analyses, vague attributions, em dash overuse, rule of three, AI vocabulary, passive voice, negative parallelisms, and filler phrases. Based on Wikipedia's "Signs of AI writing" guide.
metadata:
  version: "2.9.2"
---
...
This skill lives at `.cursor/skills/humanizer/SKILL.md` in the btq-article-maker project. Cursor loads it when you work in this repo.

**How to invoke:** Ask the agent to humanize text or remove AI writing patterns. Examples:

- "Humanize this paragraph: ..."
- "Humanize the prose in `articles/draft.md`"
- "Polish this blog post so it doesn't sound AI-generated"

**File mode in Cursor:** Read the target file, run the draft → audit → final loop internally, then write the final rewrite back with Write or StrReplace. Humanize prose only: leave code blocks, YAML frontmatter, structured data, and link targets untouched. In chat, summarize what changed instead of pasting the full rewrite unless the user asks.
...
**File mode.** The user points at a file path. In Cursor, use Read to load the file, run the draft → audit → final loop internally, then save with Write or StrReplace so the file contains only the final rewrite. Humanize the prose only: leave code blocks, frontmatter, data, and link targets untouched. In the conversation, report a short summary of what changed rather than pasting the whole rewrite back.
